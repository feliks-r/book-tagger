        ...
        <div className="absolute z-20 mt-1 w-full rounded border bg-background shadow max-h-48 overflow-y-auto">
          ...
          {!isLoading && results.length === 0 && (
            <div className="px-3 py-2 text-sm text-muted-foreground">
              No matches found. Press Enter or Tab to use as new.
            </div>
          )}
          {!isLoading &&
            results.map((item) => (
...
export default function AddBookPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      resetForm();
      setMessage({ type: "success", text: "Your book suggestion has been submitted and will be reviewed by a moderator." });
    } catch (err) {
