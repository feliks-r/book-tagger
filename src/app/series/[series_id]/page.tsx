...
import { formatAuthors, parseAuthorsJoin } from "@/lib/authors";
...
export default async function SeriesPage({ params }: PageProps) {
  ...
  const { data: rawBooks, error: booksError } = await supabase
    .from("books")
    .select("id, title, description, publication_year, cover_id, series_index, book_authors(display_order, authors(id, name))")
  ...
  const seriesBooks: Book[] = (rawBooks || []).map((b: any) => ({
  ...b,
    ...
    <div className="mx-auto max-w-4xl p-2 md:p-8 space-y-6">
      ...
      <div className="flex flex-col gap-4">
        {seriesBooks.map((book) => (
          ...
          >
            {/* Index number */}
            <div className="flex items-start pt-1 shrink-0 w-3">
